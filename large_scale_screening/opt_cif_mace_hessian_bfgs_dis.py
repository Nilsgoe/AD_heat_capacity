...
mace_calc = mace_mp(model="medium", dispersion=True, default_dtype="float64",device='cuda:0', )
...
for i,cif in enumerate(cifs):#[::10]):#cifs[:2])
    ...
    try:
        ...
        optimizer.run(fmax=0.005, steps=2500)
    except Exception as e:
        print(f"Optimization failed: {e}")
    if optimizer.get_number_of_steps() >= 2500:
        print(f"Taking the structure after {2500} steps.")
        opt=False
    with open(f"{args.dir}_mace_bfgs.csv", mode="a", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([cif, opt])
    
    write(f"opt_cif_mace_hessian_bfgs_dis/{cif.split('.')[0]}_mace_bfgs_dis.traj",atoms)
